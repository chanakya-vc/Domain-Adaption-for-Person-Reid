...
sys.path.insert(0, '.')
sys.path.append('/home/anurags/Desktop/personreid/person-reid-triplet-loss-baseline/')
import torch
...
from tri_loss.model.optimization import optimization
...
def main():
  ...
  for ep in range(start_ep, cfg.total_epochs):
    ...
    feature_list=[]
    if(ep>5):
      Z=optimization(np.array(feature_list))
    while not epoch_done:
      ...
      optimizer.zero_grad()
      if(ep>5):
        Zloss=torch.norm(feat-torch.matmul(feat,Z[(step-1)*feat.shape[1]:step*feat.shape[1],:]))
        loss = loss + cfg.beta*Zloss
      loss.backward()
      optimizer.step()
    ...
    # Epoch Log #
    #############
    #optimization call for Z over all features
