...
def extend_to_topk(ret,topk):
    tail=ret[-1]*np.ones(topk-ret.shape[0])
    return np.concatenate((ret,tail),axis=None)
...
def cmc_prid(
  ...
  print("matches.shape is: ",matches.shape," and the rank 1 is: ",np.sum(matches[:,0]==True)/m)
  matches=matches[:,:topk]
  for j in range(1,topk):
      matches[:,j]=matches[:,j]|matches[:,j-1]
  ret=np.sum(matches, axis=0)/m
  return ret
  ...
  for i in range(m):
      ...
      for j in range(n):
          if matches[i][j]==True:
               ...
               aps[i]+=(count_positive_samples/(j+1))
  return float(np.sum(aps)) / m
